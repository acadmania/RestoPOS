        ...
        <div class="row">
          ...
          <table class="table table-hover">
            ...
            <tbody>
              ...
              <tr>
                <td>Total</td>
                <td>{{Object.values(this.resultQuery).reduce((total, obj) => parseFloat(obj.quantity) + total,0) }} </td>
                <td>{{Object.values(this.resultQuery).reduce((total, obj) => parseFloat(obj.cost) + total,0) }} </td>
            </tr>
            </tbody>
          </table>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            generate(){
              this.$Progress.start();
                this.form.post('/api/report/product/sale')
                .then((response)=>{
