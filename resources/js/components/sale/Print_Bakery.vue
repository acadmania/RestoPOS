<template>
  <section class="content black-text" style="font-size:18px;padding-right:1mm;    font-family: arial;
    font-weight: 600;">
...
<hr style="background-color: black;margin:0">
  ...
  <div class="col-6" style="text-align:left;font-size:14px">
    <span v-if="sale.dateTime">Booking </span>Date: {{sale.created_at | moment }}
    <!-- Time: {{sale.created_at | time }} <br> -->
    <span v-if="sale.dateTime"> Delivery Date: {{sale.dateTime | moment }} </span>

  </div>
  <div class="col-6" style="text-align:right">
    <span v-if="$route.params.return">Return </span> Bill No: <span v-if="daily_last_sale_number"> {{ daily_last_sale_number }} </span> <span v-else> {{sale.id}} </span>  <br>
