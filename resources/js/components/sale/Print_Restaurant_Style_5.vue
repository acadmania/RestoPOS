<template>
    ...
    <div class="row">
        <div class="col 12" style="text-align:center;line-height:18px">
          ...
          <br><span v-if="settings.tagline" style="font-size:20px;"><b>{{settings.tagline}}</b></span>
      <br><span style="font-size:17px;"><b>{{settings.business_name}}</b></span>
      <br>
      ...
      <span  style="font-size:13px " v-if="settings.gst_number">GST NO: {{settings.gst_number}}</span>
    </div>
    </div>
  <hr style="background-color: black;margin:0">
    ...
    <div class="col 6" style="font-size:16px;text-align:right">
  ...
  <span v-if="sale.customer.id!==settings.customer_id">Name: {{sale.customer.name}}<br v-if="sale.customer.address"> {{sale.customer.address}}<br v-if="sale.customer.phone"> {{sale.customer.phone}} </span>
  ...
  <div v-if="sale.note" class="row" style="line-height:18px">
    ...
    <table class="col-12" style="font-size:12px">
      <thead style="border-bottom:solid black 1px;border-top:solid black 1px;">
        <tr>
          ...
          <th style="width:2%;text-align:left;padding-left: 0;padding-right: 0;">No.</th>
          <th style="width:50%;text-align:left;padding-left: 0;padding-right: 0;">Item</th>
      ...
      <tbody>
        <tr v-for="(item, index) in sale.items" :key="item.id">
         ...
         <td  style="width:11%;text-align:right">{{item.price | currency}}</td>
         <td  style="width:8%;text-align:center">{{parseFloat(item.quantity).toFixed(0)}}</td>
         <td style="width:20%;text-align:right">{{item.price*parseFloat(item.quantity)| currency}}</td>
       </tr>
      </tbody>
    </table>
    <hr style="background-color: black;margin:0">
    ...
    <div class="col 6" style="font-size:13px">
      ...
      Total Qty: {{sale.items.reduce((total, obj) => parseFloat(obj.quantity) + total,0)}}<br>
      <span v-if="sale.pay_statuses && sale.pay_statuses[0]">
        By: {{ sale.pay_statuses[0].payment_method }}
      </span>    </div>
    <!-- <div class="col 6 " style="text-align:right;font-size:15px">
      <span v-if="sale.discount > 0">Sub</span> Total: {{Math.round(sale.total) | currency}}<br>
      <span v-if="sale.discount > 0">Discount: {{sale.discount | currency}}<br>
      Total: {{Math.round(sale.grand_total) | currency}}</span>
    </div> -->
    <div class="col 6 " style="text-align:right;line-height:normal">
      <span style="font-size:13px">Sub Total: {{Math.round(sale.total)-Math.round(sale.gst) | currency}}<br></span>
      <span style="font-size:13px"> CGST 2.5% {{ sale.gst/2  | currency }}</span><br>
      <span style="font-size:13px"> SGST 2.5% {{ sale.gst/2  | currency }}</span><br>
      <span v-if="sale.discount > 0" style="font-size:13px">{{sale.total | currency}}<br></span>
      <span v-if="sale.discount > 0" style="font-size:13px">Discount: {{sale.discount | currency}}<br></span>
      <span style="font-size:15px"><b>Total: {{Math.round(sale.grand_total) | currency}}</b></span>
    </div>
    </div>
    ...
    <hr style="background-color: black;margin:0" v-if="settings.gst_number && settings.gst_summary">
      ...
      <p style="font-size:11px;text-align: center;margin-bottom:0px">{{ settings.print_footer_message }}</p>
      <hr style="background-color: black;margin:0">
      </span>

      <!-- <hr style="background-color: black;margin:0"> -->
      <span style="font-size:12px">Software By: {{ settings.brand_name }}</span>
    </div>
  </section>
  </template>
  <script>
