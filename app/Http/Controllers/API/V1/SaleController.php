...
class SaleController extends BaseController
{
  ...
  {
    if ($customer_id) {
      $items = $this->item->with('payStatuses')->where('customer_id', $customer_id)->latest('id')->paginate(10);
    } else {
      $items = $this->item->with('payStatuses')->latest('id')->paginate(10);
    }
    ...
    $response = DB::transaction(
      function () use (&$request) {
        ...
        //   }
          ...
          $transaction->opening_balance = $account->_balance;
  ...
  {
    $items = $this->item->where('id', $search)->with('payStatuses')->latest()->paginate(10);
  ...
  public function filterSale(Request $request)
  {
    ...
    $sale = $sale->with('payStatuses')->where('created_at', '>=', $from)->where('created_at', '<=', $to);

    if($request->customer)
    $sale = $sale->with('payStatuses')->where('customer_name','like', '%'.$request->customer.'%');

    if($request->type)
    $sale = $sale->with('payStatuses')->where('type','like', '%'.$request->type.'%');

    if($request->payment_status)
    $sale = $sale->with('payStatuses')->where('payment_status',$request->payment_status);


    $sale = $sale->get();

    // if($request->dateFilter !== null AND $request->dateFilter[1] !== null AND $request->customer !== null AND $request->type !== null AND $request->payment_status !== null)
    // {
    ...
    //   $sale = Sale::where('created_at', '>=', $from)->where('created_at', '<=', $to)->where('customer_name','like', '%'.$request->customer.'%')->where('type','like', '%'.$request->type.'%')->where('payment_status','like', '%'.$request->payment_status.'%')->get();
    ...
    //   $sale = Sale::where('created_at', '>=', $from)->where('created_at', '<=', $to)->where('type','like', '%'.$request->type.'%')->where('payment_status','like', '%'.$request->payment_status.'%')->get();
  ...
  public function getSaleDailyCount($id)
  {

    $from = Carbon::now()->startOfDay();
    $to = Carbon::now()->endOfDay();
    return Sale::where('created_at', '>=', $from)->where('created_at', '<=', $to)->get()->count();
  ...
  public function kotDirectPrint($order_id, $type)
  {
    ...
    if($settings->is_direct_file_print)
    {

    }
    else
    {
        $this->kotDirectPrintNoFile($order_id,$type);
        return;
    }
    $customer_name = null;
    if ($type == 'complete') {
    ...
    if($type == 'complete')
      ...
      if($settings->reset_print_number_daily)
      {
        $dailyCount = $this->getSaleDailyCount($sale->id);
      }
      else
      {
        $dailyCount =  $sale->id;
      }
      $mpdf->WriteHTML("ORDER NUMBER: <span style='font-size:100px'> <b> $dailyCount </b> </span> \n");
  ...
  public function kotDirectPrintNoFile($order_id, $type)
  {
